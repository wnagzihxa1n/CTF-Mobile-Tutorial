...
输入用户名和密码，长度要相等，用户名和密码的逆序串相等则输出字符串
...
有两处看起来值得注意，一个是`assets`文件夹，另一个是签名文件夹下的`y`

熟悉Dex文件结构的同学应该很清楚这是`abc`文件其实就是Dex的`Dex Header`，长度是`0x70`

![](Image/4.png)

再看`y`，一眼看不出是什么，我一开始猜想`y`会不会是Dex后面的数据，出题人把它拆分了，但是怎么都合并不上

![](Image/5.png)

到这里，思路真的就断了，当我看着`y`发呆的时候，突然注意到`CERT.RSA`文件的大小不正常
...
由于一直对前面的`Dex Header`念念不忘，**抱着打死出题人也要找出Dex后面数据的决心**，我做了一个大胆的猜测，Dex后面的数据肯定在这里！！！！！！
...
接下来看到`DEX=`，这个意味着后面的数据就是`Dex Header`以后的数据，再结合前面的KEY，说明这一片数据应该是被加密了，`Dex Header`后面是`String Ids`，是一片很整齐的数据，而这里乱七八糟的，明显是加密了
...
将解密后的的数据拼接到abc文件后面，使用JEB打开，提示不是有效Dex文件，其它工具解析也是类似错误

使用010Editor的模板功能分析，红框内全是0，需要我们手动修复

![](Image/10.png)

首先我们找一个正常的dex.classes文件，用010Editor解析
...
前面那些可以不在意，我们关注`_ids size`和`_ids off`这种类型的数据
...
发现`onCreate()`方法的代码是`nop`，鉴于其它方法都是正常的，那么这里明显是被抽取指令，猜测可能是运行时动态恢复指令，但是这里没有so，明显不是
